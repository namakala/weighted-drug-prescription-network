...
The inverted log approach takes the quotient of the baseline weight $\omega_B$ and the absolute-log weight $\omega_L$, or more formally $\omega_I = \frac{\omega_B}{\omega_L}$. The main motivation of using an inverted log approach is to increase the weight given that DDD $\neq$ 1. Since it uses $\omega_L$ to calculate the weight, it shares the same limitation as well. For the absolute log approach with the log base is set to 10, DDD of 0.1 will return $\omega_L$ = 0, which results in $\omega_I$ = $\infty$. Since it violates the second criteria, all values returning $\infty$ is changed as 0. Despite its limitation, the inverted log approach is usable to increase the weight when DDD $\neq$ 1.
...
| Inverted $\omega_L$ | $\omega_I = \frac{\omega_B}{\omega_L}$ | Put more weight for DDD other than 1 |
...
```{r}
#| fig-cap: Simulation on all weighting approaches
#| fig-height: 2
#| fig-width: 10
